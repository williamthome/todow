...
% All rights reserved.
% @see https://github.com/redink/task/blob/master/LICENSE
-module(todow_task).

-export([
    async/1, async/2, async/3, async/4,
    async_opt/2, async_opt/3, async_opt/4, async_opt/5,
    async_do/3,
    await/1, await/2,
    safe_await/2, safe_await/3
]).
...
await({Pid, Ref}, TimeOut) ->
    receive
        ...
        {'DOWN', Ref, _, _, noconnection} ->
            erlang:exit({nodedown, erlang:node(Pid), {?MODULE, await, [{Pid, Ref}, TimeOut]}});
        {'DOWN', Ref, _, _, Reason} ->
            erlang:exit({Reason, {?MODULE, await, [{Pid, Ref}, TimeOut]}})
    after TimeOut ->
        erlang:demonitor(Ref, [flush]),
        erlang:exit({timeout, {?MODULE, await, [{Pid, Ref}, TimeOut]}})
    end.
...
do_apply(TaskOwnerInfo, {Mod, Fun, Args} = MFA) ->
    try
        ...
        error:Value:Stacktrace ->
            task_exit(TaskOwnerInfo, MFA, {Value, Stacktrace});
        throw:Value:Stacktrace ->
            task_exit(TaskOwnerInfo, MFA, {{nocatch, Value}, Stacktrace});
        exit:Value ->
...
task_exit(_, _, Reason) when
    erlang:tuple_size(Reason) =:= 2 andalso
